...
from representation_method import malconv,n_gram_1,n_gram_2,n_gram_3,split_basicblock_byte,instruction_count,Asm2VecPlus,Asm2VecBase,Magic,Asm2VecBase1,\
    representation_method_func,Asm2VecBase_get_basicblock_seq_vec_list,Asm2VecPlus_get_basicblock_seq_vec_list,get_basicblock_seq_vec_list,Asm2VecPlus_get_basicblock_seq_vec_list_8,\
...
def get_sample_list_from_csv(csv_save_path="../data/malware_msg_last.csv"):
    file_list = []
    with open(csv_save_path, 'r', encoding='latin-1') as f:
...
if __name__ == '__main__':
    ...
    begnin_shaixuan_list = list(get_sample_list_from_csv(csv_save_path=r"\\ZJNU-NSR\Malware\SOREL-20M\benign_all.csv"))[20009]
    # print(len(begnin_shaixuan_list))
    # exit()
    # for i in range(len(begnin_shaixuan_list)):
    #     begnin_shaixuan_list[i]=begnin_shaixuan_list[i].replace(".gexf","")

    # featured_cfg_mp(mulu, ["init_vector_374"])


    # folder_path = r'\\ZJNU-NSR\Malware\Microsoft\train_exe'  # 替换为您的文件夹路径
    output_csv_path = r'\\ZJNU-NSR\Malware\SOREL-20M\CFG_benign\edges.csv'  # 替换为您的输出CSV文件路径
    ...
    get_edges_msg(folder_path,begnin_shaixuan_list,output_csv_path)
