...
def process_edge_row(row):
    CFG_Basicblock_Byte_Sequence = ast.literal_eval(row['Basicblock Byte Sequence'])
    File_name=row['File Name']
    return File_name,CFG_Basicblock_Byte_Sequence
...
def read_cfg_from_csv_mutiple_thread(path=r"\\ZJNU-NSR\Malware\Microsoft\CFG",num_samples=None):
    ...
    with open(os.path.join(path, "edges.csv"), 'r') as edges_file:
        ...
        with mp.Pool(processes=8) as pool:
            ...
            try:
                for idx, (File_name,CFG_Basicblock_Byte_Sequence) in enumerate(tqdm(results)):
    ...
    return File_names,CFG_Basicblock_Byte_Sequence_list


def load_CFG_Basicblock_Byte_Sequence_list(folder, num_samples=None):
    File_names,CFG_Basicblock_Byte_Sequence_list = read_cfg_from_csv_mutiple_thread(path=folder,num_samples=num_samples)
    return File_names,CFG_Basicblock_Byte_Sequence_list
...
def representation_method_func(basicblock_byte_seq,representation_method,offspring=1,instructions_in_vertex=1):
    ...
    if representation_method=="malconv":
        ...
        lena=len(basicblock_byte_seq)
        if lena > 100:
            Asm2VecBase_8_vec_list, Asm2VecBase_16_vec_list, Asm2VecBase_32_vec_list, Asm2VecBase_64_vec_list, Asm2VecBase_128_vec_list, Asm2VecBase_256_vec_list = Asm2VecBase(
                hex_asm=basicblock_byte_seq)
        else:
            Asm2VecBase_8_vec_list, Asm2VecBase_16_vec_list, Asm2VecBase_32_vec_list, Asm2VecBase_64_vec_list, Asm2VecBase_128_vec_list, Asm2VecBase_256_vec_list = Asm2VecBase1(
                hex_asm=basicblock_byte_seq)
...
if __name__ == '__main__':
    ...
    # featured_cfg(folder_path,malware_shaixuan_list,mulu,["Magic"])
